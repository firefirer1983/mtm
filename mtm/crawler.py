...
class Crawler:
    ...
    def _setup(self, *argv):
        ...
        if opts.batchfile is not None:
            try:
                ...
                sys.exit(
                    "ERROR: batch file %s could not be read" % opts.batchfile
                )
        self._all_urls = batch_urls + [
        ...
        if opts.list_extractors:
            for ie in list_extractors(opts.age_limit):
                ...
                matchedUrls = [
                    url for url in self._all_urls if ie.suitable(url)
                ]
        ...
        if opts.http_chunk_size is not None:
            numeric_chunksize = FileDownloader.parse_bytes(
                opts.http_chunk_size
            )
        ...
        if (
            opts.playlistend not in (-1, None)
            and opts.playlistend < opts.playliststart
        ):
            raise ValueError(
        ...
        if opts.embedthumbnail:
            already_have_thumbnail = (
                opts.writethumbnail or opts.write_all_thumbnails
            )
            postprocessors.append(
                {
                    "key": "EmbedThumbnail",
        ...
        if opts.exec_cmd:
            postprocessors.append(
                {"key": "ExecAfterDownload", "exec_cmd": opts.exec_cmd,}
            )
    ...
    def _crawl(self, argv=None):
        with YoutubeDL(self._ydl_opts) as ydl:
            # Update version
            if self._opts.update_self:
                update_self(ydl.to_screen, self._opts.verbose, ydl._opener)

            # Remove cache dir
            if self._opts.rm_cachedir:
                ydl.cache.remove()

            # Maybe do nothing
            if (len(self._all_urls) < 1) and (
                self._opts.load_info_filename is None
            ):
                if self._opts.update_self or self._opts.rm_cachedir:
                    sys.exit()
            ...
            try:
                if self._opts.load_info_filename is not None:
                    retcode = ydl.download_with_info_file(
                        expand_path(self._opts.load_info_filename)
                    )
                else:
                    retcode = ydl.download(self._all_urls)
