...
from mtm.components.cache_manager import Material
from mtm.components.downloader import Downloader
from mtm.utils.string_fmt import is_partial_dir

CACHE_REPO = "/home/xy/misc/cache/youtube"
download_urls = [
...
def get_resume_urls():
    urls = []
    dirs = []
    for sub_dir in Path(CACHE_REPO).iterdir():
        if is_partial_dir(sub_dir):
            continue
        else:
            cache = Material(sub_dir)
            if cache.is_complete:
                continue
            dirs.append(str(sub_dir))
            urls.append(
